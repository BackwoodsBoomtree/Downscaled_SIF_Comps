...
#### Output PDF name ####
out_name   <- "G:/SIF_comps/figs/v2/regressions_seasia_v2.pdf"
...
# Labels
lab_title   <- "Asia-Pacific Tropical Forest"
...
axis(1, tck = 0.03, mgp=c(3, 0.2, 0))
axis(2, tck = 0.03, mgp=c(3, 0.2, 0), las = 2)
legend("topleft", bty = "n", legend = c(as.expression(reg_nir_cs_cold_r), as.expression(reg_nir_cs_cold_p)))
...
axis(1, tck = 0.03, mgp=c(3, 0.2, 0))
axis(2, tck = 0.03, mgp=c(3, 0.2, 0), las = 2)
legend("topleft", bty = "n", legend = c(as.expression(reg_nir_cf_cold_r), as.expression(reg_nir_cf_cold_p)))
mtext(1, text = lab_781, line = 1.75)
box()
