...
cf20_diff  <- "G:/SIF_comps/min_max_diff/20km/cf20_vis/NIRv_Rad.CF20-SIF.max.CF20.monthly.20km.2020.tif"
...
#### Get the data ####
nirv_cf     <- crop(mask(rast(cf_file, subds = "NIRv"), m), africa_ext)
nirv_rad_cf <- crop(mask(rast(cf_file, subds = "NIRv_Rad"), m), africa_ext)
sif_cf      <- crop(mask(rast(cf_file, subds = "SIF_743"), m), africa_ext)
ref_665_cf  <- crop(mask(rast(cf_file, subds = "REF_665"), m), africa_ext)
ref_781_cf  <- crop(mask(rast(cf_file, subds = "REF_781"), m), africa_ext)

n_cf            <- crop(mask(rast(cf_file, subds = "n"), m), africa_ext)
nirv_std_cf     <- crop(mask(rast(cf_file, subds = "NIRv_std"), m), africa_ext)
nirv_rad_std_cf <- crop(mask(rast(cf_file, subds = "NIRv_Rad_std"), m), africa_ext)
sif_std_cf      <- crop(mask(rast(cf_file, subds = "SIF_743_std"), m), africa_ext)
ref_665_std_cf  <- crop(mask(rast(cf_file, subds = "REF_665_std"), m), africa_ext)
ref_781_std_cf  <- crop(mask(rast(cf_file, subds = "REF_781_std"), m), africa_ext)

nirv_cs     <- crop(mask(rast(cs_file, subds = "NIRv"), m), africa_ext)
nirv_rad_cs <- crop(mask(rast(cs_file, subds = "NIRv_Rad"), m), africa_ext)
sif_cs      <- crop(mask(rast(cs_file, subds = "SIF_743"), m), africa_ext)
ref_665_cs  <- crop(mask(rast(cs_file, subds = "REF_665"), m), africa_ext)
ref_781_cs  <- crop(mask(rast(cs_file, subds = "REF_781"), m), africa_ext)

n_cs            <- crop(mask(rast(cs_file, subds = "n"), m), africa_ext)
nirv_std_cs     <- crop(mask(rast(cs_file, subds = "NIRv_std"), m), africa_ext)
nirv_rad_std_cs <- crop(mask(rast(cs_file, subds = "NIRv_Rad_std"), m), africa_ext)
sif_std_cs      <- crop(mask(rast(cs_file, subds = "SIF_743_std"), m), africa_ext)
ref_665_std_cs  <- crop(mask(rast(cs_file, subds = "REF_665_std"), m), africa_ext)
ref_781_std_cs  <- crop(mask(rast(cs_file, subds = "REF_781_std"), m), africa_ext)
...
#### Plot ####
cairo_pdf("G:/SIF_comps/figs/comps_africa_map_black.pdf", width = 7.5, height = 4.25)
...
plot(raster(cover), legend.only=TRUE, col=map.cols, horizontal=F,
     ...
     axis.args = list(line = -3.25, cex.axis=1, tick=F, at=c(80.3, 100), labels=c("80","100"), col.axis = "white"),
