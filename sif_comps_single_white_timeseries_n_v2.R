...
out_name    <- "G:/SIF_comps/figs/v2/sif_comps_single_white_timeseries_n_v2.pdf"
...
tropo_sif_n <- tropo_sif_data$n
oco2_sif_n  <- oco2_sif_data$n
oco3_sif_n  <- oco3_sif_data$n
gome2_sif_n  <- gome2_sif_data$n

# expand data for plotting for 2011 - 2022
tropo_sif_n <- c(rep(NA, 96), tropo_sif_data$n)
oco2_sif_n  <- c(rep(NA, 48), oco2_sif_data$n)
oco3_sif_n  <- c(rep(NA, 96), oco3_sif_data$n)
...
y_n        <- c(2000, 2200000)
y_nirv     <- c(0.25, 0.33)
y_lab_sif  <- bquote("Number of Soundings")
y_labs     <- c(bquote("2 x 10"^"3"), bquote("2 x 10"^"4"), bquote("2 x 10"^"5"), bquote("2 x 10"^"6"))
...
plot(NA, xlim = x_len, ylim = y_n, log = "y", col = nirv.col, axes = FALSE, xaxs="i", type = "o", lwd = 2, pch = 4, cex = 0.75, lty = 1)

lines(gome2_sif_n, col = sifg.col, type = "o", lwd = 1, pch = 17, cex = 0.75, lty = 1)
lines(tropo_sif_n, col = sift.col, type = "o", lwd = 1, pch = 16, cex = 0.75, lty = 1)
lines(oco2_sif_n, col = sifo2.col, type = "o", lwd = 1, pch = 15, cex = 0.75, lty = 1)
lines(oco3_sif_n, col = sifo3.col, type = "o", lwd = 1, pch = 18, cex = 0.75, lty = 1)
axis(2, tck = 0.03, mgp=c(3, 0.2, 0), las = 2, labels = as.expression(y_labs), at = c(2000, 20000, 200000, 2000000))
...
legend("topleft", legend = c("GOME-2", "TROPOMI", "OCO-2", "OCO-3"), ncol = 3, lwd = c(1,1,1,1),
