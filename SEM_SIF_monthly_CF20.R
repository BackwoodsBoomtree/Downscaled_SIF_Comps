...
std_cf20_mar_sif <- mask(rast("G:/TROPOMI/esa/gridded/20km/monthly/2020/TROPOMI.ESA.SIF.2020.global.monthly.20km.CF20.nc", subds = "SIF_743_std")[[3]], m)
cf20_mar_sif <- mask(rast("G:/TROPOMI/esa/gridded/20km/monthly/2020/TROPOMI.ESA.SIF.2020.global.monthly.20km.CF20.nc", subds = "SIF_743")[[3]], m)
sem_cf20_mar_sif <- std_cf20_mar_sif / sqrt(n_cf20_mar)
...
sem_cf20_mar_sif       <- raster(sem_cf20_mar_sif)
...
sem_diff <- sem_cf20_mar_sif - sem_cs_mar_sif
...
sem_cf20_mar_sif_median <- as.vector(sem_cf20_mar_sif)
sem_cf20_mar_sif_median <- sem_cf20_mar_sif_median[!is.na(sem_cf20_mar_sif_median)]
sem_cf20_mar_sif_median <- sort(sem_cf20_mar_sif_median)
sem_cf20_mar_sif_median <- median(sem_cf20_mar_sif_median)
...
labs <- c(paste0("Clearsky Number of Soundings per Gridcell"),
          ...
          paste0("SIF CF ", intToUtf8(8804), "0.20 Standard Error of Mean"),
          paste0("Difference in Number of Soundings (c - a)"),
          paste0("Difference in Standard Error of Mean (d - b)"))

##### PLOTS ####

cairo_pdf("G:/SIF_comps/figs/SEM_SIF_monthly_CF20_black.pdf", width=7.5, height=6)
...
par(new=TRUE)
op <- par(mar = c(3.1,0.75,8,21)) # Set margins
hist(n_cf20_mar, col=n.col, breaks=10, ylim=c(0,175000), xlim=c(0,200), xaxs="i", yaxs="i", ann=FALSE, axes=FALSE, border = NA)
rect(par("usr")[1],par("usr")[3],par("usr")[2],par("usr")[4], col = "black", border = "white")
...
hist(n_cf20_mar, col=n.col, breaks=10, ylim=c(0,175000), xlim=c(0,200), xaxs="i", yaxs="i", ann=FALSE, axes=FALSE, border = NA, add=T)
...
### SEM CF20
op <- par(mar = c(0,0,0.25,0.25), bg = "black")
sem_cf20_mar_sif[sem_cf20_mar_sif < 0]   <- 0
sem_cf20_mar_sif[sem_cf20_mar_sif > 0.2] <- 0.2
plot(sem_cf20_mar_sif, ext=c(-180,180,-80,80), axes=F, xaxs="i", yaxs="i", horizontal=T, legend=F, col = NA)
plot(coastlines, add = TRUE, border = NA, col = rgb(0.30,0.30,0.30))
plot(sem_cf20_mar_sif, col=sem.col, ext=c(-180,180,-80,80), axes=F, xaxs="i", yaxs="i", horizontal=T, legend=F, add = TRUE)
rect(par("usr")[1],par("usr")[3],par("usr")[2],par("usr")[4],col = NA, border = "white")
mtext(3, text=labs[4], cex=0.85, col = "white")
mtext(3, text="d", cex= 0.85, adj=0, font=2, col = "white")

plot(sem_cf20_mar_sif, legend.only=TRUE, col=sem.col, horizontal=T, legend.width=2, legend.shrink=0.75,
     legend.args = list(text=expression(paste("Standard Error of Mean")), side = 1, line = -1.65, cex=0.75, col = "white"),
     axis.args = list(line = -1.05, cex.axis=1,tick=F, at=c(0,0.1,0.2), labels=c("0","0.1",">0.2"), col.axis = "white"),
     smallplot=c(0.40,0.90,0.2,0.25)); par(mar = par("mar"))
...
par(new=TRUE)
op <- par(mar = c(3.1,0.75,8,21)) # Set margins
hist(sem_cf20_mar_sif, col=sem.col, breaks=10, ylim=c(0,175000), xlim=c(0,0.2), xaxs="i", yaxs="i", ann=FALSE, axes=FALSE, border = NA)
rect(par("usr")[1],par("usr")[3],par("usr")[2],par("usr")[4], col = "black", border = "white")
abline(v=sem_cf20_mar_sif_median, col="red")
axis(3, tck=F, labels = round2(sem_cf20_mar_sif_median, 3), at = sem_cf20_mar_sif_median, mgp=c(3, 0.1, 0), col.axis = "white")
hist(sem_cf20_mar_sif, col=sem.col, breaks=10, ylim=c(0,175000), xlim=c(0,0.2), xaxs="i", yaxs="i", ann=FALSE, axes=FALSE, border = NA, add=T)
