...
#### Output PDF name ####
# out_name   <- "G:/SIF_comps/figs/comps_africa_map_3yr_black.pdf"
# out_name   <- "G:/SIF_comps/figs/comps_africa_map_3yr_cold_black.pdf"
# out_name   <- "G:/SIF_comps/figs/comps_africa_map_3yr_strict_black.pdf"
# out_name   <- "G:/SIF_comps/figs/comps_africa_map_3yr_cold_strict_black.pdf"
out_name   <- "G:/SIF_comps/figs/comps_africa_map_3yr_cold_all_black.pdf"
...
cover        <- crop(m, africa_ext)

#### Get the data ####
n_cf        <- crop(mask(c(rast(cf_2019, subds = "n"), rast(cf_2020, subds = "n"), rast(cf_2021, subds = "n")), m), africa_ext)
nirv_cf     <- crop(mask(c(rast(cf_2019, subds = "NIRv"), rast(cf_2020, subds = "NIRv"), rast(cf_2021, subds = "NIRv")), m), africa_ext)
nirv_rad_cf <- crop(mask(c(rast(cf_2019, subds = "NIRv_Rad"), rast(cf_2020, subds = "NIRv_Rad"), rast(cf_2021, subds = "NIRv_Rad")), m), africa_ext)
sif_cf      <- crop(mask(c(rast(cf_2019, subds = "SIF_743"), rast(cf_2020, subds = "SIF_743"), rast(cf_2021, subds = "SIF_743")), m), africa_ext)
ref_665_cf  <- crop(mask(c(rast(cf_2019, subds = "REF_665"), rast(cf_2020, subds = "REF_665"), rast(cf_2021, subds = "REF_665")), m), africa_ext)
ref_781_cf  <- crop(mask(c(rast(cf_2019, subds = "REF_781"), rast(cf_2020, subds = "REF_781"), rast(cf_2021, subds = "REF_781")), m), africa_ext)

nirv_std_cf     <- crop(mask(c(rast(cf_2019, subds = "NIRv_std"), rast(cf_2020, subds = "NIRv_std"), rast(cf_2021, subds = "NIRv_std")), m), africa_ext)
nirv_rad_std_cf <- crop(mask(c(rast(cf_2019, subds = "NIRv_Rad_std"), rast(cf_2020, subds = "NIRv_Rad_std"), rast(cf_2021, subds = "NIRv_Rad_std")), m), africa_ext)
sif_std_cf      <- crop(mask(c(rast(cf_2019, subds = "SIF_743_std"), rast(cf_2020, subds = "SIF_743_std"), rast(cf_2021, subds = "SIF_743_std")), m), africa_ext)
ref_665_std_cf  <- crop(mask(c(rast(cf_2019, subds = "REF_665_std"), rast(cf_2020, subds = "REF_665_std"), rast(cf_2021, subds = "REF_665_std")), m), africa_ext)
ref_781_std_cf  <- crop(mask(c(rast(cf_2019, subds = "REF_781_std"), rast(cf_2020, subds = "REF_781_std"), rast(cf_2021, subds = "REF_781_std")), m), africa_ext)

n_cs        <- crop(mask(c(rast(cs_2019, subds = "n"), rast(cs_2020, subds = "n"), rast(cs_2021, subds = "n")), m), africa_ext)
nirv_cs     <- crop(mask(c(rast(cs_2019, subds = "NIRv"), rast(cs_2020, subds = "NIRv"), rast(cs_2021, subds = "NIRv")), m), africa_ext)
nirv_rad_cs <- crop(mask(c(rast(cs_2019, subds = "NIRv_Rad"), rast(cs_2020, subds = "NIRv_Rad"), rast(cs_2021, subds = "NIRv_Rad")), m), africa_ext)
sif_cs      <- crop(mask(c(rast(cs_2019, subds = "SIF_743"), rast(cs_2020, subds = "SIF_743"), rast(cs_2021, subds = "SIF_743")), m), africa_ext)
ref_665_cs  <- crop(mask(c(rast(cs_2019, subds = "REF_665"), rast(cs_2020, subds = "REF_665"), rast(cs_2021, subds = "REF_665")), m), africa_ext)
ref_781_cs  <- crop(mask(c(rast(cs_2019, subds = "REF_781"), rast(cs_2020, subds = "REF_781"), rast(cs_2021, subds = "REF_781")), m), africa_ext)

nirv_std_cs     <- crop(mask(c(rast(cs_2019, subds = "NIRv_std"), rast(cs_2020, subds = "NIRv_std"), rast(cs_2021, subds = "NIRv_std")), m), africa_ext)
nirv_rad_std_cs <- crop(mask(c(rast(cs_2019, subds = "NIRv_Rad_std"), rast(cs_2020, subds = "NIRv_Rad_std"), rast(cs_2021, subds = "NIRv_Rad_std")), m), africa_ext)
sif_std_cs      <- crop(mask(c(rast(cs_2019, subds = "SIF_743_std"), rast(cs_2020, subds = "SIF_743_std"), rast(cs_2021, subds = "SIF_743_std")), m), africa_ext)
ref_665_std_cs  <- crop(mask(c(rast(cs_2019, subds = "REF_665_std"), rast(cs_2020, subds = "REF_665_std"), rast(cs_2021, subds = "REF_665_std")), m), africa_ext)
ref_781_std_cs  <- crop(mask(c(rast(cs_2019, subds = "REF_781_std"), rast(cs_2020, subds = "REF_781_std"), rast(cs_2021, subds = "REF_781_std")), m), africa_ext)

n_cs_all        <- crop(mask(c(rast(cs_all_2019, subds = "n"), rast(cs_all_2020, subds = "n"), rast(cs_all_2021, subds = "n")), m), africa_ext)
nirv_cs_all     <- crop(mask(c(rast(cs_all_2019, subds = "NIRv"), rast(cs_all_2020, subds = "NIRv"), rast(cs_all_2021, subds = "NIRv")), m), africa_ext)
nirv_rad_cs_all <- crop(mask(c(rast(cs_all_2019, subds = "NIRv_Rad"), rast(cs_all_2020, subds = "NIRv_Rad"), rast(cs_all_2021, subds = "NIRv_Rad")), m), africa_ext)
sif_cs_all      <- crop(mask(c(rast(cs_all_2019, subds = "SIF_743"), rast(cs_all_2020, subds = "SIF_743"), rast(cs_all_2021, subds = "SIF_743")), m), africa_ext)
ref_665_cs_all  <- crop(mask(c(rast(cs_all_2019, subds = "REF_665"), rast(cs_all_2020, subds = "REF_665"), rast(cs_all_2021, subds = "REF_665")), m), africa_ext)
ref_781_cs_all  <- crop(mask(c(rast(cs_all_2019, subds = "REF_781"), rast(cs_all_2020, subds = "REF_781"), rast(cs_all_2021, subds = "REF_781")), m), africa_ext)

nirv_std_cs_all     <- crop(mask(c(rast(cs_all_2019, subds = "NIRv_std"), rast(cs_all_2020, subds = "NIRv_std"), rast(cs_all_2021, subds = "NIRv_std")), m), africa_ext)
nirv_rad_std_cs_all <- crop(mask(c(rast(cs_all_2019, subds = "NIRv_Rad_std"), rast(cs_all_2020, subds = "NIRv_Rad_std"), rast(cs_all_2021, subds = "NIRv_Rad_std")), m), africa_ext)
sif_std_cs_all      <- crop(mask(c(rast(cs_all_2019, subds = "SIF_743_std"), rast(cs_all_2020, subds = "SIF_743_std"), rast(cs_all_2021, subds = "SIF_743_std")), m), africa_ext)
ref_665_std_cs_all  <- crop(mask(c(rast(cs_all_2019, subds = "REF_665_std"), rast(cs_all_2020, subds = "REF_665_std"), rast(cs_all_2021, subds = "REF_665_std")), m), africa_ext)
ref_781_std_cs_all  <- crop(mask(c(rast(cs_all_2019, subds = "REF_781_std"), rast(cs_all_2020, subds = "REF_781_std"), rast(cs_all_2021, subds = "REF_781_std")), m), africa_ext)
...
mtext(3, text = "Africa Tropical Forest", col = "white")
box(col = "white")
# Legend
