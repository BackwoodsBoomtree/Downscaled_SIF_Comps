...
#### Plot ####
cairo_pdf("G:/SIF_comps/figs/comps_africa_map_3yr_black.pdf", width = 7.5, height = 4.25)
...
plot(raster(cover), legend.only=TRUE, col=map.cols, horizontal=F,
     ...
     axis.args = list(line = -3.25, cex.axis=1, tick=F, at=c(81, 100), labels=c("80","100"), col.axis = "white"),
...
plot(x, ts_sif_cf, col = mag.cols[4], type = "l", axes = FALSE, lwd = 1.5, xaxs="i",
     ylim = c(min(ts_sif_cs, ts_sif_cf) - 0.025 * min(ts_sif_cs, ts_sif_cf),
              max(ts_sif_cs, ts_sif_cf) + 0.045 * max(ts_sif_cs, ts_sif_cf)))
