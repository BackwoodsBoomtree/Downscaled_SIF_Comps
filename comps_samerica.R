...
nirv_rad_cf <- crop(mask(rast(cf_file, subds = "NIRv_Rad"), m), samerica_ext)
sif_cf      <- crop(mask(rast(cf_file, subds = "SIF_743"), m), samerica_ext)
ref_665_cf  <- crop(mask(rast(cf_file, subds = "REF_665"), m), samerica_ext)
ref_781_cf  <- crop(mask(rast(cf_file, subds = "REF_781"), m), samerica_ext)

n_cf            <- crop(mask(rast(cf_file, subds = "n"), m), samerica_ext)
nirv_std_cf     <- crop(mask(rast(cf_file, subds = "NIRv_std"), m), samerica_ext)
nirv_rad_std_cf <- crop(mask(rast(cf_file, subds = "NIRv_Rad_std"), m), samerica_ext)
sif_std_cf      <- crop(mask(rast(cf_file, subds = "SIF_743_std"), m), samerica_ext)
ref_665_std_cf  <- crop(mask(rast(cf_file, subds = "REF_665_std"), m), samerica_ext)
ref_781_std_cf  <- crop(mask(rast(cf_file, subds = "REF_781_std"), m), samerica_ext)

nirv_cs     <- crop(mask(rast(cs_file, subds = "NIRv"), m), samerica_ext)
nirv_rad_cs <- crop(mask(rast(cs_file, subds = "NIRv_Rad"), m), samerica_ext)
sif_cs      <- crop(mask(rast(cs_file, subds = "SIF_743"), m), samerica_ext)
ref_665_cs  <- crop(mask(rast(cs_file, subds = "REF_665"), m), samerica_ext)
ref_781_cs  <- crop(mask(rast(cs_file, subds = "REF_781"), m), samerica_ext)

n_cs            <- crop(mask(rast(cs_file, subds = "n"), m), samerica_ext)
nirv_std_cs     <- crop(mask(rast(cs_file, subds = "NIRv_std"), m), samerica_ext)
nirv_rad_std_cs <- crop(mask(rast(cs_file, subds = "NIRv_Rad_std"), m), samerica_ext)
sif_std_cs      <- crop(mask(rast(cs_file, subds = "SIF_743_std"), m), samerica_ext)
ref_665_std_cs  <- crop(mask(rast(cs_file, subds = "REF_665_std"), m), samerica_ext)
ref_781_std_cs  <- crop(mask(rast(cs_file, subds = "REF_781_std"), m), samerica_ext)
...
#### Plot ####
cairo_pdf("G:/SIF_comps/figs/comps_samerica_map_black.pdf", width = 7.5, height = 4.25)
...
lines(x, ts_sif_cs, col = line.cols[2], lwd = 1.5, lty = 2)
